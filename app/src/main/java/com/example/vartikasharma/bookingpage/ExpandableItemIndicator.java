...
public class ExpandableItemIndicator extends FrameLayout {
    ...
    private void onInit(Context context, AttributeSet attrs, int defStyleAttr) {
        if (shouldUseAnimatedIndicator(context, attrs, defStyleAttr)) {
            impl = new ExpandableItemIndicatorImplAnim();
        } else {
            impl = new ExpandableItemIndicatorImpNoAnim();
        }
        impl.onInit(context, attrs, defStyleAttr, this);
    }
